...
#pagesModeHint{display:none;flex:1;align-items:center;justify-content:center;color:#cbd5e1;font-size:14px}
body.modePages #centerTop{display:none}
body.modePages #canvasWrap{display:none}
body.modePages #pagesModeHint{display:flex}
body.modeEditor #pagesModeHint{display:none}
body.modeEditor #centerTop{display:flex}
body.modeEditor #canvasWrap{display:flex}
...
#editorPanel .bar{flex:0 0 auto;display:flex;gap:6px;flex-wrap:wrap}
...
#editorPanel .bar select{background:#0f172a;color:#e5e7eb;border:1px solid #273446;border-radius:6px;padding:6px 8px}
#editorPanel .bar button{background:#1f2937;border:1px solid #273446;color:#e5e7eb;border-radius:6px;padding:6px 8px;cursor:pointer}
...
let maxW=Math.max(100,wrap.clientWidth);
let maxH=Math.max(100,wrap.clientHeight);
let s=Math.min(maxW/cw,maxH/ch);
s=Math.max(0.05,Math.min(10,s));
elCanvas.style.transform="scale("+s+")";
}
...
</html>""" der acktuelle code aber erstens ist der teil links(editor) noch gequetscht. und 2. das vorschau past auch nicht (hier als referenz eine ganz alte version wo die vorschau so umgefär gepasst hat: """<?php
header("Content-Type:text/html;charset=utf-8");
$path=__DIR__."/data.json";
$allow_private_fetch=false;
if(($_GET["action"]??"")==="fetchHtml"){
...
$p["widgets"]=array_values(array_filter($p["widgets"],fn($w)=>is_array($w)&&isset($w["id"])&&isset($w["type"])));
foreach($p["widgets"] as &$w){
$w=deep_defaults($w,["id"=>$w["id"]??"w".bin2hex(random_bytes(4)),"type"=>$w["type"]??"text","x"=>40,"y"=>40,"w"=>320,"h"=>180,"z"=>1,"locked"=>false,"hidden"=>false,"contentW"=>3840,"contentH"=>2160,"style"=>["radius"=>0,"shadow"=>false,"bg"=>"","color"=>"","font"=>16],"src"=>"","mode"=>"iframe","text"=>"","format"=>"HH:mm:ss","playlist"=>[],"duration"=>5,"title"=>""]);
if(!isset($w["contentW"])||!is_numeric($w["contentW"]))$w["contentW"]=3840;
if(!isset($w["contentH"])||!is_numeric($w["contentH"]))$w["contentH"]=2160;
if(!is_array($w["style"]))$w["style"]=["radius"=>0,"shadow"=>false,"bg"=>"","color"=>"","font"=>16];
if(!is_array($w["playlist"]))$w["playlist"]=[];
...
#centerTop .pill{font-size:12px;color:#111827;background:#e5e7eb;border:1px solid #d1d5db;border-radius:999px;padding:5px 10px}
#canvasWrap{flex:1;position:relative;background:#0b1220;overflow:hidden}
...
let cw=+p.settings.w||3840,ch=+p.settings.h||2160;
let maxW=Math.max(20,cw-(w.x||0)),maxH=Math.max(20,ch-(w.y||0));
w.w=Math.round(clamp(resizeState.startW+dw,20,maxW));
w.h=Math.round(clamp(resizeState.startH+dh,20,maxH));
renderCanvas();
}
}
function onUp(){dragState=null;resizeState=null;document.onmousemove=null;document.onmouseup=null;}
