...
#canvasScale{position:relative;overflow:hidden}
#canvas{position:absolute;left:0;top:0;transform-origin:0 0;margin:0;background:#111827;border:0;border-radius:6px;outline:2px solid #334155;outline-offset:0;box-shadow:0 8px 24px rgba(0,0,0,.35)}
#bgPreview{position:absolute;inset:0;border-radius:6px;overflow:hidden;pointer-events:none;z-index:0}
...
<div class=sp></div>
<button id=toggleBg type=button>BG</button>
...
let showBgInEditor=false;
function applyBgVis(){
let off=(uiMode==="editor"&&!showBgInEditor);
elBgPreview.style.display=off?"none":"block";
elBgPreview.style.opacity=(uiMode==="editor"&&!off)?"0.25":"1";
}
document.getElementById("toggleBg").onclick=()=>{
showBgInEditor=!showBgInEditor;
applyBgVis();
renderCanvas();
};
...
let viewScale=1;
function applyViewScale(){let p=getPage();let cw=+p.settings.w||3840,ch=+p.settings.h||2160;let wrap=document.getElementById("canvasWrap");let scaleBox=document.getElementById("canvasScale");let padR=0,padB=0;
let maxW=Math.max(100,wrap.clientWidth-padR);
let maxH=Math.max(100,wrap.clientHeight-padB);let s=Math.min(maxW/cw,maxH/ch);s=Math.max(0.05,Math.min(10,s));scaleBox.style.width=Math.round(cw*s)+"px";scaleBox.style.height=Math.round(ch*s)+"px";elCanvas.style.transform="scale("+s+")";elCanvas.style.transformOrigin="0 0";viewScale=s;}
...
if(!data.meta)data.meta={timezone:"Europe/Berlin",autosync_hours:6,autosync_seconds:30,globalRotationSpeed:1,carouselIframeRefreshSeconds:0};
else{
if(data.meta.autosync_seconds==null)data.meta.autosync_seconds=30;
if(data.meta.carouselIframeRefreshSeconds==null)data.meta.carouselIframeRefreshSeconds=0;
}
...
applyBgVis();
if(!(uiMode==="editor"&&!showBgInEditor))startBgPreview(p);
else{elBgA.classList.remove("on");elBgB.classList.remove("on");}
...
let p=getPage();
p.widgets.forEach(w=>{
let el=elCanvas.querySelector('.widget[data-id="'+cssEsc(w.id)+'"]');
if(!el)return;
el.style.left=(w.x||0)+"px";
el.style.top=(w.y||0)+"px";
el.style.width=(w.w||100)+"px";
el.style.height=(w.h||80)+"px";
el.classList.toggle("sel",selectedIds.includes(w.id));
...
function tickPreview(){let p=getPage();if(dragState||resizeState){tickClock();return;}tickBg(p);tickClock();tickCarousel(p);}
...
function itemRefreshSeconds(it){
let r=it&&(it.refresh??it.reload??it.refreshSeconds??it.refresh_seconds??0);
r=+r||0;
if(r>0)return r;
let g=+((data.meta&&data.meta.carouselIframeRefreshSeconds)||0)||0;
return g>0?g:0;
}
function pvEnsureNode(st,idx){
...
node.className="slot";
node.dataset.src0=String(it.src||"");
node.dataset.isIframe=(t!=="image")?"1":"0";
node.dataset.refresh=String(itemRefreshSeconds(it)||0);
node.dataset.lastReload="0";
st.box.appendChild(node);
st.nodes[idx]=node;
return node;
}
function pvShow(st,idx){
if(!st||!st.list||!st.list.length)return;
idx=((idx%st.list.length)+st.list.length)%st.list.length;
if(st.activeNode)st.activeNode.classList.remove("on");
let n=pvEnsureNode(st,idx);
n.classList.add("on");
st.activeNode=n;
st.i=idx;
}
function pvTickRefresh(st){
if(dragState||resizeState)return;
let n=st.activeNode;
if(!n||n.dataset.isIframe!=="1")return;
let r=+n.dataset.refresh||0;
if(r<=0)return;
let last=+n.dataset.lastReload||0;
let now=Date.now();
if(last===0){n.dataset.lastReload=String(now);return;}
if(now-last<r*1000)return;
n.dataset.lastReload=String(now);
let src0=n.dataset.src0||n.src||"";
n.src=withBust(src0);
}
function renderCarouselItem(el,w,i){
let list=Array.isArray(w.playlist)?w.playlist:[];
if(!list.length){el.innerHTML="";return;}
let sig=list.map(it=>String(it.type||"image")+"|"+String(it.src||"")+"|"+(+it.duration||0)+"|"+(+((it.refresh??it.reload??it.refreshSeconds??0))||0)).join("||");
let st=carState[w.id];
if(!st||st.sig!==sig||st.box!==el){carState[w.id]=st={sig:sig,i:0,nextAt:0,list:list,nodes:[],activeNode:null,box:el};el.innerHTML="";}
pvShow(st,i);
}
function tickCarousel(p){
let now=Date.now();
(p.widgets||[]).filter(w=>w&&w.type==="carousel"&&Array.isArray(w.playlist)&&w.playlist.length).forEach(w=>{
let el=document.querySelector('.widget[data-id="'+cssEsc(w.id)+'"] .pvCarousel');
if(!el)return;
let st=carState[w.id];
if(!st||st.box!==el){renderCarouselItem(el,w,0);st=carState[w.id];}
pvTickRefresh(st);
if(st.nextAt===0){st.nextAt=now+clamp(+((st.list[st.i]||{}).duration||5),1,3600)*1000;return;}
if(now<st.nextAt)return;
pvShow(st,(st.i+1)%st.list.length);
st.nextAt=now+clamp(+((st.list[st.i]||{}).duration||5),1,3600)*1000;
});
}
