...
.pg .acts{display:flex;gap:4px;flex:0 0 auto;margin-left:auto}
...
body.modeEditor #pages{display:none}
body.modePages #side{display:none}
</style></head><body>
<div id=top>
...
let pad=24;
let maxW=Math.max(100,wrap.clientWidth-pad);
let maxH=Math.max(100,wrap.clientHeight-pad);
let s=Math.min(maxW/cw,maxH/ch,1);
...
rrBtns.appendChild(btn("Up","alt",()=>moveRule(idx,-1)));
rrBtns.appendChild(btn("Down","alt",()=>moveRule(idx,1)));
rrBtns.appendChild(btn("Duplicate","alt",()=>{let c=deepClone(r);c.id=uid();c.name=c.name+" Copy";data.schedules.splice(idx+1,0,c);renderSide();}));
rrBtns.appendChild(btn("Delete","danger",()=>{data.schedules.splice(idx,1);renderSide();}));
