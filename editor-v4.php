...
function updateCanvasRects(){let p=getPage();p.widgets.forEach(w=>{let el=elCanvas.querySelector('.widget[data-id="'+cssEsc(w.id)+'"]');if(!el)return;el.style.left=(w.x||0)+"px";el.style.top=(w.y||0)+"px";el.style.width=(w.w||100)+"px";el.style.height=(w.h||80)+"px";el.classList.toggle("sel",selectedIds.includes(w.id));applyWidgetStyle(el,w);});elSelInfo.textContent=selectedIds.length?("Selected: "+selectedIds.length):"Selected:
