...
$default=["meta"=>["timezone"=>"Europe/Berlin","autosync_hours"=>6,"globalRotationSpeed"=>1],"pages"=>[["id"=>"p1","name"=>"Page 1","thumb"=>"","settings"=>["w"=>1920,"h"=>1080],"background"=>["fade"=>1,"items"=>[]],"widgets"=>[]]],"schedules"=>[]];
...
$w=deep_defaults($w,["id"=>$w["id"]??"w".bin2hex(random_bytes(4)),"type"=>$w["type"]??"text","x"=>40,"y"=>40,"w"=>240,"h"=>140,"z"=>1,"locked"=>false,"hidden"=>false,"style"=>["radius"=>0,"shadow"=>false,"bg"=>"","color"=>"","font"=>16],"src"=>"","text"=>"","format"=>"HH:mm:ss","playlist"=>[],"duration"=>5,"title"=>""]);
...
#canvasWrap{flex:1;position:relative;background:#0b1220;overflow:auto}
#canvas{position:relative;margin:18px auto;background:#111827;border:1px solid #1f2a44;border-radius:10px}
#bgPreview{position:absolute;inset:0;border-radius:10px;background:#0b1220 center/cover no-repeat;opacity:.45;pointer-events:none}
...
<div id=canvasWrap><div id=canvas><div id=bgPreview></div></div></div>
</div>
<div id=side>
<div id=tabs>
...
if(!data.pages.length)data.pages=[{id:"p1",name:"Page 1",thumb:"",settings:{w:1920,h:1080},background:{fade:1,items:[]},widgets:[]}];
...
function addPage(){
data.pages.push({id:uid(),name:"Page "+(data.pages.length+1),thumb:"",settings:{w:1920,h:1080},background:{fade:1,items:[]},widgets:[]});
...
let bg=p.background.items[0]&&p.background.items[0].src?p.background.items[0].src:"";
elBgPreview.style.backgroundImage=bg?"url('"+bg.replace(/'/g,"%27")+"')":"none";
...
let base={id:uid(),type,x:60,y:60,w:320,h:180,z:nextZ(),locked:false,hidden:false,style:{radius:8,shadow:false,bg:"rgba(255,255,255,.84)",color:"#111827",font:16},src:"",text:"",format:"HH:mm:ss",playlist:[]};
if(type==="text"){base.text="Hello";}
if(type==="clock"){base.w=220;base.h=90;base.style.font=28;base.style.bg="rgba(17,24,39,.65)";base.style.color="#fff";base.style.radius=14;base.style.shadow=true;}
if(type==="image"){base.src="https://";}
if(type==="url"){base.src="https://";}
if(type==="carousel"){base.playlist=[{type:"image",src:"https://",duration:5,title:""}];}
...
w.x=Math.round(s.x+dx);
w.y=Math.round(s.y+dy);
});
renderCanvas();
}
if(resizeState){
...
w.w=Math.round(clamp(resizeState.startW+dw,20,99999));
w.h=Math.round(clamp(resizeState.startH+dh,20,99999));
renderCanvas();
}
}
...
let r=document.createElement("div");r.className="row2";
let z=document.createElement("div");z.appendChild(label("Z"));z.appendChild(input("number",w.z,v=>{w.z=clamp(+v||1,1,99999);renderCanvas();}));
...
if(w.type==="url"){cfg.appendChild(label("URL (iframe)"));cfg.appendChild(input("text",w.src,v=>{w.src=v;renderCanvas();}));}
