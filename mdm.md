...
document.getElementById("btnAddWidget").onclick=()=>popShow(document.getElementById("btnAddWidget"));
...
let row=document.createElement("div");row.className="cRow";wrap.appendChild(row);
let sw=document.createElement("div");sw.className="cSw";row.appendChild(sw);
let col=document.createElement("input");col.type="color";row.appendChild(col);
let aW=document.createElement("div");aW.className="cA";row.appendChild(aW);
let a=document.createElement("input");a.type="range";a.min="0";a.max="1";a.step="0.01";aW.appendChild(a);
let pct=document.createElement("div");pct.className="cPct";row.appendChild(pct);
let defRow=document.createElement("div");defRow.className="cDef";wrap.appendChild(defRow);
let def=document.createElement("input");def.type="checkbox";defRow.appendChild(def);
let defLab=document.createElement("span");defLab.className="small";defLab.textContent="Default";defRow.appendChild(defLab);
function setUiEnabled(on){
col.disabled=!on;
a.disabled=!on;
}
function paint(){
if(def.checked){sw.style.setProperty("--c","transparent");pct.textContent="";onChange("");return;}
...
onChange(av===0?"":rgba(rgb.r,rgb.g,rgb.b,av));
}
let cur=String(current||"").trim();
if(!cur){def.checked=true;col.value="#ffffff";a.value="1";setUiEnabled(false);paint();}
else{
def.checked=false;
...
def.oninput=()=>{setUiEnabled(!def.checked);paint();};
col.oninput=paint;
a.oninput=paint;
return wrap;
}
...
st.appendChild(rs);
st.appendChild(colorPickerRow("Background",w.style.bg,v=>{w.style.bg=v;renderCanvas();}));
st.appendChild(colorPickerRow("Text Color",w.style.color,v=>{w.style.color=v;renderCanvas();}));
...
elPanel.appendChild(f);elPanel.appendChild(st);elPanel.appendChild(cfg);
}
function addRule(){
...
</html>""" funktionaler als davor(""" """) aber immernoch zb add button auserhalb (es ist einfach uneinheitlich unübersichtlich und inkonsitent(die farbauswahl...?)) am besten wären mehr ausklapp/phantom menüss und alles an richtigen orten sauber sortirte eindeutige funktionen
""""""""""
""""""""""
gib mir entsprechend den alten anforderungen basirend auf dem alten code welche code blöcke wie ersetzen/einfügen usw
