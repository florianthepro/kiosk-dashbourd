...
#modePill{background:#111827;color:#fff;border-color:#3a414a}
body.modePages #modePill{background:#7c2d12;border-color:#fb923c;color:#fff}
body.modeEditor #modePill{background:#064e3b;border-color:#34d399;color:#fff}
#top #topSave,#top #topExport,#top #topImport{background:#2d333b;border:1px solid #3a414a}
#popAdd{position:fixed;z-index:9999;display:none;width:min(360px,92vw);background:#fff;border:1px solid #d1d5db;border-radius:10px;box-shadow:0 14px 40px rgba(0,0,0,.35)}
#popAdd .h{padding:10px 12px;border-bottom:1px solid #e5e7eb;font-weight:700}
#popAdd .b{padding:10px 12px}
#popAdd .f{display:flex;gap:8px;padding:10px 12px;border-top:1px solid #e5e7eb}
#popAdd .f button{flex:1;background:#111827;color:#fff;border:1px solid #111827;border-radius:8px;padding:8px 10px;cursor:pointer}
#popCancel{background:#fff!important;color:#111827!important;border-color:#d1d5db!important}
.cRow{display:flex;gap:8px;align-items:center}
.cSw{width:22px;height:22px;border-radius:6px;border:1px solid #d1d5db;position:relative;overflow:hidden;flex:0 0 auto}
.cSw:before{content:"";position:absolute;inset:0;background:linear-gradient(45deg,#cbd5e1 25%,transparent 25%),linear-gradient(-45deg,#cbd5e1 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#cbd5e1 75%),linear-gradient(-45deg,transparent 75%,#cbd5e1 75%);background-size:10px 10px;background-position:0 0,0 5px,5px -5px,-5px 0}
.cSw:after{content:"";position:absolute;inset:0;background:var(--c,transparent)}
.cA{flex:1}
.cA input[type=range]{width:100%}
.cPct{width:44px;text-align:right;font-size:12px;color:#6b7280}
.cDef{display:flex;align-items:center;gap:8px;margin-top:6px}
#popBg{position:fixed;z-index:9999;display:none;width:min(560px,94vw);max-height:min(76vh,720px);background:#fff;border:1px solid #d1d5e7;border-radius:10px;box-shadow:0 14px 40px rgba(0,0,0,.35);overflow:hidden}
#popBg .h{padding:10px 12px;border-bottom:1px solid #e5e7eb;font-weight:700}
#popBg .b{padding:10px 12px;overflow:auto;max-height:calc(min(76vh,720px) - 104px)}
#popBg .f{display:flex;gap:8px;padding:10px 12px;border-top:1px solid #e5e7eb}
#popBg .f button{flex:1;background:#111827;color:#fff;border:1px solid #111827;border-radius:8px;padding:8px 10px;cursor:pointer}
.btn{background:#111827;color:#fff;border:1px solid #111827;border-radius:8px;padding:8px 10px;cursor:pointer}
.btn:hover{filter:brightness(1.05)}
.btn.alt{background:#fff;color:#111827;border-color:#d1d5db}
.btn.danger{background:#ef4444;border-color:#ef4444}
.sec{border:1px solid #e5e7eb;border-radius:10px;margin:0 0 10px;background:#fff}
.sec>summary{cursor:pointer;list-style:none;padding:10px 12px;font-weight:800;color:#111827}
.sec>summary::-webkit-details-marker{display:none}
.sec .secBody{padding:10px 12px;border-top:1px solid #e5e7eb}
...
<span id=modePill class=badge>Editor</span>
<button id=topSave type=button>Save</button>
<button id=topExport type=button>Export</button>
<button id=topImport type=button>Import</button>
<div class=sp></div>
...
<div id=popAdd>
<div class=h>Add Widget</div>
<div class=b></div>
<div class=f>
<button id=popCancel type=button>Cancel</button>
<button id=popOk type=button>OK</button>
</div>
</div>
<div id=popBg>
<div class=h>Edit Background</div>
<div class=b></div>
<div class=f>
<button id=popBgClose type=button>Close</button>
</div>
</div>
...
const elPop=document.getElementById("popAdd");
const elPopBody=elPop.querySelector(".b");
const elPopCancel=document.getElementById("popCancel");
const elPopOk=document.getElementById("popOk");
const elPopBg=document.getElementById("popBg");
const elPopBgBody=elPopBg.querySelector(".b");
const elPopBgClose=document.getElementById("popBgClose");
function popBgHide(){elPopBg.style.display="none";}
function popBgShow(anchorEl){
elPopBgBody.innerHTML="";
let p=getPage();
let top=document.createElement("div");top.className="row2";
let a=document.createElement("div");a.appendChild(label("Fade seconds"));a.appendChild(input("number",p.background.fade,v=>{p.background.fade=clamp(+v||0,0,20);renderCanvas();}));
let b=document.createElement("div");b.appendChild(label("Add item"));let add=document.createElement("button");add.type="button";add.className="btn";add.textContent="+ Image";add.onclick=()=>{p.background.items.push({src:"https://",duration:10});popBgShow(anchorEl);renderCanvas();};b.appendChild(add);
top.appendChild(a);top.appendChild(b);
elPopBgBody.appendChild(top);
let items=p.background.items||[];
items.forEach((it,i)=>{
let box=document.createElement("fieldset");box.style.borderStyle="dashed";
let lg=document.createElement("legend");lg.textContent="Item "+(i+1);box.appendChild(lg);
box.appendChild(label("Image URL"));box.appendChild(input("text",it.src||"",v=>{it.src=v;renderPages();renderCanvas();}));
box.appendChild(label("Duration (seconds)"));box.appendChild(input("number",it.duration||10,v=>{it.duration=clamp(+v||1,1,3600);}));
let br=document.createElement("div");br.className="btnRow";
br.appendChild(btn("Up","alt",()=>{if(i===0)return;let t=items[i-1];items[i-1]=items[i];items[i]=t;popBgShow(anchorEl);renderCanvas();}));
br.appendChild(btn("Down","alt",()=>{if(i===items.length-1)return;let t=items[i+1];items[i+1]=items[i];items[i]=t;popBgShow(anchorEl);renderCanvas();}));
br.appendChild(btn("Delete","danger",()=>{items.splice(i,1);popBgShow(anchorEl);renderCanvas();}));
box.appendChild(br);
elPopBgBody.appendChild(box);
});
let r=anchorEl.getBoundingClientRect();
let w=560,h=420;
let x=Math.min(window.innerWidth-w-8,Math.max(8,r.left));
let y=Math.min(window.innerHeight-h-8,Math.max(56,r.bottom+8));
elPopBg.style.left=x+"px";
elPopBg.style.top=y+"px";
elPopBg.style.display="block";
}
elPopBgClose.onclick=()=>popBgHide();
document.addEventListener("mousedown",e=>{if(elPopBg.style.display!=="none"&&!elPopBg.contains(e.target)&&e.target.id!=="btnEditBg")popBgHide();});
document.addEventListener("keydown",e=>{if(e.key==="Escape")popBgHide();});
let popType="text";
function popHide(){elPop.style.display="none";}
function popShow(anchorEl){
elPopBody.innerHTML="";
let s=document.createElement("select");
[{v:"text",t:"Text"},{v:"image",t:"Image"},{v:"url",t:"URL"},{v:"clock",t:"Clock"},{v:"carousel",t:"Carousel"}].forEach(o=>{let op=document.createElement("option");op.value=o.v;op.textContent=o.t;s.appendChild(op);});
s.value=popType;s.oninput=()=>popType=s.value;
elPopBody.appendChild(label("Type"));
elPopBody.appendChild(s);
let r=anchorEl.getBoundingClientRect();
let x=Math.min(window.innerWidth-elPop.offsetWidth-8,Math.max(8,r.left));
let y=Math.min(window.innerHeight-220,Math.max(56,r.bottom+8));
elPop.style.left=x+"px";
elPop.style.top=y+"px";
elPop.style.display="block";
}
elPopCancel.onclick=()=>popHide();
elPopOk.onclick=()=>{popHide();addWidget(popType);};
document.addEventListener("mousedown",e=>{if(elPop.style.display!=="none"&&!elPop.contains(e.target)&&e.target.id!=="btnAddWidget")popHide();});
...
document.getElementById("btnAddWidget").onclick=()=>{if(uiMode!=="editor")return;popShow(document.getElementById("btnAddWidget"));};
document.getElementById("topSave").onclick=()=>doSave();
document.getElementById("topExport").onclick=()=>downloadJson();
document.getElementById("topImport").onclick=()=>importJsonPrompt();updateWidgetToolbar();
...
if(typeof popHide==="function")popHide();
if(typeof popBgHide==="function")popBgHide();
...
if(mp)mp.textContent=(uiMode==="pages")?"Pages":"Editor";
renderAll();
}
...
function hexToRgb(h){h=String(h||"").trim();let m=h.match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i);if(!m)return null;let x=m[1];if(x.length===3)x=x.split("").map(c=>c+c).join("");let n=parseInt(x,16);return {r:(n>>16)&255,g:(n>>8)&255,b:n&255};}
function rgba(r,g,b,a){a=clamp(+a||0,0,1);a=Math.round(a*100)/100;return "rgba("+Math.round(r)+","+Math.round(g)+","+Math.round(b)+","+a+")";}
function parseAlpha(v){let s=String(v||"");let m=s.match(/rgba?\(\s*[\d.]+\s*,\s*[\d.]+\s*,\s*[\d.]+(?:\s*,\s*([\d.]+))?\s*\)/i);if(!m)return 1;return clamp(m[1]==null?1:+m[1],0,1);}
function parseRgb(v){
let s=String(v||"").trim();
let m=s.match(/rgba?\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)/i);
if(m)return {r:clamp(+m[1]||255,0,255),g:clamp(+m[2]||255,0,255),b:clamp(+m[3]||255,0,255)};
let h=s.match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i);
if(h){let rgb=hexToRgb(h[0]);if(rgb)return rgb;}
return {r:255,g:255,b:255};
}
function rgbToHex(r,g,b){let t=x=>("0"+(x|0).toString(16)).slice(-2);return "#"+t(r)+t(g)+t(b);}
function colorPickerRow(title,current,onChange){
let wrap=document.createElement("div");
wrap.appendChild(label(title));
let row=document.createElement("div");row.className="row2";wrap.appendChild(row);
let left=document.createElement("div");row.appendChild(left);
let right=document.createElement("div");row.appendChild(right);
let cRow=document.createElement("div");cRow.className="cRow";left.appendChild(cRow);
let sw=document.createElement("div");sw.className="cSw";cRow.appendChild(sw);
let col=document.createElement("input");col.type="color";cRow.appendChild(col);
let defRow=document.createElement("div");defRow.className="cDef";left.appendChild(defRow);
let def=document.createElement("input");def.type="checkbox";defRow.appendChild(def);
let defLab=document.createElement("span");defLab.className="small";defLab.textContent="Use default";defRow.appendChild(defLab);
right.appendChild(label("Opacity"));
let a=document.createElement("input");a.type="range";a.min="0";a.max="1";a.step="0.01";right.appendChild(a);
let pct=document.createElement("div");pct.className="small";pct.style.marginTop="6px";right.appendChild(pct);
function setEnabled(on){col.disabled=!on;a.disabled=!on;}
let init=true;
function apply(){
if(def.checked){
sw.style.setProperty("--c","transparent");
pct.textContent="Default";
if(!init)onChange("");
return;
}
let rgb=hexToRgb(col.value)||{r:255,g:255,b:255};
let av=clamp(+a.value,0,1);
sw.style.setProperty("--c",rgba(rgb.r,rgb.g,rgb.b,av));
pct.textContent=Math.round(av*100)+"%";
if(!init)onChange(av===0?"":rgba(rgb.r,rgb.g,rgb.b,av));
}
...
def.checked=false;
let av=parseAlpha(cur);a.value=String(av);
let rgb=parseRgb(cur);col.value=rgbToHex(rgb.r,rgb.g,rgb.b);
setEnabled(true);
apply();
}
init=false;
def.oninput=()=>{setEnabled(!def.checked);apply();};
col.oninput=apply;
a.oninput=apply;
return wrap;
}
...
elSelInfo.textContent=selectedIds.length?("Selected: "+selectedIds.length):"Selected: none";updateWidgetToolbar();
}
...
let d=document.createElement("details");d.className="sec";if(open)d.open=true;
let s=document.createElement("summary");s.textContent=title;d.appendChild(s);
let b=document.createElement("div");b.className="secBody";d.appendChild(b);
d.body=b;return d;
}
function updateWidgetToolbar(){
let hasSel=selectedIds.length>0;
let bF=document.getElementById("btnFront");
let bB=document.getElementById("btnBack");
let bD=document.getElementById("btnDup");
let bX=document.getElementById("btnDel");
if(bF)bF.disabled=!hasSel;
if(bB)bB.disabled=!hasSel;
if(bD)bD.disabled=!hasSel;
if(bX)bX.disabled=!hasSel;
}
function renderPage(){
let p=getPage();
...
let fbg=fieldset("Background");
let row=document.createElement("div");row.className="row2";
let a=document.createElement("div");a.appendChild(label("Fade seconds"));a.appendChild(input("number",p.background.fade,v=>{p.background.fade=clamp(+v||0,0,20);renderCanvas();}));
let b=document.createElement("div");b.appendChild(label("Items"));
let stat=document.createElement("div");stat.className="small";stat.textContent=String((p.background.items||[]).length)+" item(s)";
b.appendChild(stat);
let eb=document.createElement("button");eb.type="button";eb.id="btnEditBg";eb.className="btn";eb.textContent="Edit…";eb.style.marginTop="8px";
eb.onclick=e=>{e.preventDefault();popBgShow(eb);};
b.appendChild(eb);
row.appendChild(a);row.appendChild(b);
fbg.appendChild(row);
elPanel.appendChild(f);
elPanel.appendChild(fbg);
}
...
function renderWidget(){let size=document.createElement("div");size.appendChild(label("Size (W×H)"));
let sz=document.createElement("div");sz.className="small";sz.textContent=(w.w|0)+"×"+(w.h|0);size.appendChild(sz);
r2.appendChild(hid);r2.appendChild(size);
sBasics.body.appendChild(r2);
let sStyle=sec("Style",true);
...
sStyle.body.appendChild(rs);
sStyle.body.appendChild(colorPickerRow("Background",w.style.bg,v=>{w.style.bg=v;renderCanvas();}));
sStyle.body.appendChild(colorPickerRow("Text Color",w.style.color,v=>{w.style.color=v;renderCanvas();}));
sStyle.body.appendChild(label("Font px"));
sStyle.body.appendChild(input("number",w.style.font,v=>{w.style.font=clamp(+v||16,6,200);renderCanvas();}));
let sResize=sec("Resize / Presets",false);
...
sResize.body.appendChild(brp2);
let sContent=sec("Content",true);
if(w.type==="text"){
sContent.body.appendChild(label("Text"));
sContent.body.appendChild(textarea(w.text,v=>{w.text=v;renderCanvas();}));
}else if(w.type==="image"){
sContent.body.appendChild(label("Image URL"));
sContent.body.appendChild(input("text",w.src||"",v=>{w.src=v;renderCanvas();}));
}else if(w.type==="url"){
sContent.body.appendChild(label("URL"));
sContent.body.appendChild(input("text",w.src||"",v=>{w.src=v;renderCanvas();}));
sContent.body.appendChild(label("Mode"));
sContent.body.appendChild(select(w.mode||"iframe",[{v:"iframe",t:"iframe"},{v:"fetch",t:"fetch (curl)"}],v=>{w.mode=v;renderCanvas();}));
...
cr.appendChild(cW);cr.appendChild(cH);
sContent.body.appendChild(cr);
}else if(w.type==="clock"){
sContent.body.appendChild(label("Format (stored)"));
sContent.body.appendChild(input("text",w.format||"HH:mm:ss",v=>{w.format=v;renderCanvas();}));
let sm=document.createElement("div");sm.className="small";sm.textContent="Preview uses locale time; format is stored for later.";sContent.body.appendChild(sm);
}else if(w.type==="carousel"){
sContent.body.appendChild(label("Items"));
let br=document.createElement("div");br.className="btnRow";
...
sContent.body.appendChild(br);
(w.playlist||[]).forEach((it,i)=>{
let fi=document.createElement("fieldset");fi.style.borderStyle="dashed";
let lg=document.createElement("legend");lg.textContent="Item "+(i+1);fi.appendChild(lg);
fi.appendChild(label("Type"));
fi.appendChild(select(it.type||"image",[{v:"image",t:"image"},{v:"url",t:"url"}],v=>{it.type=v;renderCanvas();}));
fi.appendChild(label("Src"));
fi.appendChild(input("text",it.src||"",v=>{it.src=v;renderCanvas();}));
fi.appendChild(label("Title (optional)"));
fi.appendChild(input("text",it.title||"",v=>{it.title=v;}));
fi.appendChild(label("Duration seconds"));
fi.appendChild(input("number",it.duration||5,v=>{it.duration=clamp(+v||1,1,3600);}));
...
sContent.body.appendChild(fi);
});
}
elPanel.appendChild(sBasics);
elPanel.appendChild(sStyle);
elPanel.appendChild(sResize);
elPanel.appendChild(sContent);
}
``
let p=getPage();
...
let t=document.createElement("div");
t.className="small";
t.textContent="Select a widget on the canvas (double click opens Widget tab).";
f.appendChild(t);
elPanel.appendChild(f);
return;
}
...
let t=document.createElement("div");
t.className="small";
t.textContent="Editing applies to all selected widgets where applicable.";
f.appendChild(t);
let s=sec("Common Style",true);
let r=document.createElement("div");r.className="row2";
...
s.body.appendChild(colorPickerRow("Background","",v=>{selectedIds.forEach(id=>{let w=getWidgetById(id);if(w){w.style=w.style||{};w.style.bg=v;}});renderCanvas();}));
s.body.appendChild(colorPickerRow("Text Color","",v=>{selectedIds.forEach(id=>{let w=getWidgetById(id);if(w){w.style=w.style||{};w.style.color=v;}});renderCanvas();}));
s.body.appendChild(label("Font px"));
s.body.appendChild(input("number","",v=>{selectedIds.forEach(id=>{let w=getWidgetById(id);if(w){w.style=w.style||{};w.style.font=+v||16;}});renderCanvas();}));
elPanel.appendChild(f);
elPanel.appendChild(s);
return;
}
let w=getWidgetById(selectedIds[0]);
if(!w){selectedIds=[];renderAll();return;}
w.style=w.style||{radius:0,shadow:false,bg:"",color:"",font:16};
let sBasics=sec("Basics",true);
sBasics.body.appendChild(label("Type"));
let ty=document.createElement("div");ty.textContent=w.type;sBasics.body.appendChild(ty);
let r=document.createElement("div");r.className="row2";
let zBox=document.createElement("div");zBox.appendChild(label("Layer (Z)"));
...
zBox.appendChild(zr);
let lockBox=document.createElement("div");lockBox.appendChild(label("Locked"));lockBox.appendChild(checkbox(!!w.locked,v=>{w.locked=v;renderCanvas();}));
r.appendChild(zBox);r.appendChild(lockBox);
sBasics.body.appendChild(r);
...
let r2=document.createElement("div");r2.className="row2";
let hid=document.createElement("div");hid.appendChild(label("Hidden"));hid.appendChild(checkbox(w.hidden,v=>{w.hidden=v;renderCanvas();}));
let size=document.createElement("div");size.appendChild(label("Size (W×H)"));let sz=document.createElement("div");sz.className="small";sz.textContent=(w.w|0)+"×"+(w.h|0);size.appendChild(sz);
r2.appendChild(hid);r2.appendChild(size);
sBasics.body.appendChild(r2);
let sStyle=sec("Style",true);
let rs=document.createElement("div");rs.className="row2";
