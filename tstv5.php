...
#modalMask{position:fixed;inset:0;background:rgba(0,0,0,.35);z-index:9998;display:none}
.modal{position:fixed;left:50%;top:72px;transform:translateX(-50%);z-index:9999;width:min(560px,94vw);max-height:80vh;background:#fff;border:1px solid #d1d5db;border-radius:12px;box-shadow:0 18px 54px rgba(0,0,0,.35);display:none}
.modal .b{overflow:auto;max-height:calc(80vh - 120px)}
</style>
...
const elMask=document.getElementById("modalMask");
function modalShow(el){elMask.style.display="block";el.style.display="block"}
function modalHide(el){el.style.display="none";if(!document.querySelector(".modal[style*='block']"))elMask.style.display="none"}
elMask.onclick=()=>{document.querySelectorAll(".modal").forEach(m=>m.style.display="none");elMask.style.display="none"}
document.addEventListener("keydown",e=>{if(e.key==="Escape")elMask.onclick()})
function popBgShow(){
elPopBgBody.innerHTML="";
let p=getPage();
let fade=input("number",p.background.fade,v=>{p.background.fade=clamp(v,0,20);renderCanvas()});
elPopBgBody.appendChild(label("Fade seconds"));
elPopBgBody.appendChild(fade);
p.background.items.forEach((it,i)=>{
elPopBgBody.appendChild(input("text",it.src,v=>{it.src=v;renderCanvas()}));
elPopBgBody.appendChild(input("number",it.duration,v=>{it.duration=clamp(v,1,3600)}));
});
modalShow(elPopBg);
}
elPopBgClose.onclick=()=>modalHide(elPopBg);
function popShow(){
elPopBody.innerHTML="";
["text","image","url","clock","carousel"].forEach(t=>{
...
elPopBody.appendChild(l);
});
modalShow(elPop);
}
elPopCancel.onclick=()=>modalHide(elPop);
elPopOk.onclick=()=>{modalHide(elPop);addWidget(popType)};
document.getElementById("btnAddWidget").onclick=()=>{if(uiMode==="editor")popShow()};
...
function sec(t,o){let d=document.createElement("details");d.open=o;let s=document.createElement("summary");s.textContent=t;let b=document.createElement("div");d.appendChild(s);d.appendChild(b);d.body=b;return d}
function setTab(t){
...
let init=true;
function apply(){if(init)return;onChange(a.value==0?"":rgba(...Object.values(parseRgb(c.value)),a.value))}
c.oninput=apply;a.oninput=apply;
if(current){let rgb=parseRgb(current);c.value=rgbToHex(rgb.r,rgb.g,rgb.b);a.value=parseAlpha(current)}
init=false;
wrap.appendChild(c);wrap.appendChild(a);
return wrap;
}
...
eb.onclick=e=>{e.preventDefault();popBgShow()};
b.appendChild(eb);
