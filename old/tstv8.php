...
#actionBar button{background:#111827;color:#fff;border:1px solid #1f2937;border-radius:6px;padding:6px 10px}
#actionBar button.danger{background:#7f1d1d;border-color:#991b1b}
.acc{border:1px solid #d1d5db;border-radius:8px;margin-bottom:8px}
.accHead{padding:8px;font-weight:600;background:#f3f4f6;cursor:pointer}
.accBody{display:none;padding:8px}
.acc.open .accBody{display:block}
body.modePages #canvas .widget{pointer-events:none}
body.modePages #widgetListLeft{display:none}
</style>
...
<div id=actionBar>
<button data-act=page>Add Page</button>
<button data-act=widget>Add Widget</button>
...
document.addEventListener("mousedown",e=>{if(elPop.style.display!=="none"&&!elPop.contains(e.target)&&e.target.id!=="btnAddWidget"&&e.target.id!=="topAddWidget")popHide();});
...
document.getElementById("topImport").onclick=()=>importJsonPrompt();
document.getElementById("actionBar").onclick=e=>{
let a=e.target.dataset.act;
if(!a)return;
if(a==="page")addPage();
if(a==="widget")popShow(e.target);
if(a==="dup")dupSelected();
if(a==="del")delSelected();
};
...
r.onclick=()=>{selectedIds=[w.id];renderAll();updateActionBar();};
...
bar.querySelector('[data-act="page"]').style.display = uiMode==="pages" ? "" : "none";
bar.querySelector('[data-act="widget"]').style.display = uiMode==="editor" ? "" : "none";
bar.querySelector('[data-act="dup"]').style.display = (uiMode==="editor" && selectedIds.length) ? "" : "none";
bar.querySelector('[data-act="del"]').style.display = (uiMode==="editor" && selectedIds.length) ? "" : "none";
}
...
function colorPickerRow(title,val,on){
let w=document.createElement("div");
w.appendChild(label(title));
let r=document.createElement("div");r.className="row2";
let c=document.createElement("input");c.type="color";
let a=document.createElement("input");a.type="range";a.min=0;a.max=1;a.step=0.01;
let rgb={r:255,g:255,b:255},alpha=1;
if(val&&val.startsWith("rgba")){
let m=val.match(/rgba\((\d+),(\d+),(\d+),([\d.]+)\)/);
if(m){rgb={r:+m[1],g:+m[2],b:+m[3]};alpha=+m[4];}
}
c.value="#"+[rgb.r,rgb.g,rgb.b].map(x=>("0"+x.toString(16)).slice(-2)).join("");
a.value=alpha;
c.oninput=a.oninput=()=>on(a.value==0?"":`rgba(${parseInt(c.value.slice(1,3),16)},${parseInt(c.value.slice(3,5),16)},${parseInt(c.value.slice(5,7),16)},${a.value})`);
r.appendChild(c);r.appendChild(a);w.appendChild(r);
return w;
}
function renderPages(){
elPageList.innerHTML="";
data.pages.forEach((p,i)=>{
let row=document.createElement("div");row.className="pg"+(i===pi?" active":"");row.onclick=()=>{pi=i;selectedIds=[];renderAll();updateActionBar();};
...
function movePage(i,dir){let j=i+dir;if(j<0||j>=data.pages.length)return;let a=data.pages[i];data.pages.splice(i,1);data.pages.splice(j,0,a);pi=j;renderAll();updateActionBar();}
function addPage(){
...
function dupPage(i){
let p=deepClone(data.pages[i]);p.id=uid();p.name=p.name+" Copy";p.widgets.forEach(w=>{w.id=uid();w.x+=20;w.y+=20;});data.pages.push(p);pi=data.pages.length-1;selectedIds=[];renderAll();updateActionBar();
}
function delPage(i){
if(data.pages.length<=1)return;
data.pages.splice(i,1);
if(pi>=data.pages.length)pi=data.pages.length-1;
selectedIds=[];renderAll();updateActionBar();
}
...
widgets.forEach(w=>{
if(uiMode==="pages"){
elSelInfo.textContent="Pages mode";
return;
}
...
d.ondblclick=e=>{e.stopPropagation();selectedIds=[w.id];renderAll();updateActionBar();};
...
elCanvas.onmousedown=e=>{if(e.target===elCanvas||e.target===elBgPreview){selectedIds=[];renderAll();updateActionBar();}};
function renderSide(){
elPanel.innerHTML="";
sanitizeClient();
...
a.body.appendChild(label("Name"));
a.body.appendChild(input("text",p.name,v=>{p.name=v;renderPages();renderCanvas();}));
let s=document.createElement("div");s.className="row2";
s.appendChild(input("number",p.settings.w,v=>{p.settings.w=clamp(v,320,20000);renderCanvas();}));
s.appendChild(input("number",p.settings.h,v=>{p.settings.h=clamp(v,240,20000);renderCanvas();}));
a.body.appendChild(label("Canvas W / H"));
a.body.appendChild(s);
let b=accordion("Background");
b.body.appendChild(input("number",p.background.fade,v=>{p.background.fade=clamp(v,0,20);renderCanvas();}));
let btn=document.createElement("button");
btn.textContent="Edit Background";
btn.onclick=()=>popBgShow(btn);
b.body.appendChild(btn);
...
function renderWidgetSingle(){
let w=getWidgetById(selectedIds[0]);
let a=accordion("Widget",true);
a.body.appendChild(label("Type"));
a.body.appendChild(document.createTextNode(w.type));
a.body.appendChild(checkbox(w.locked,v=>{w.locked=v;renderCanvas();}));
a.body.appendChild(checkbox(w.hidden,v=>{w.hidden=v;renderCanvas();}));
let s=accordion("Style");
s.body.appendChild(colorPickerRow("Background",w.style.bg,v=>{w.style.bg=v;renderCanvas();}));
s.body.appendChild(colorPickerRow("Text",w.style.color,v=>{w.style.color=v;renderCanvas();}));
s.body.appendChild(input("number",w.style.font,v=>{w.style.font=v;renderCanvas();}));
let c=accordion("Content");
if(w.type==="text")c.body.appendChild(textarea(w.text,v=>{w.text=v;renderCanvas();}));
if(w.type==="image"||w.type==="url")c.body.appendChild(input("text",w.src,v=>{w.src=v;renderCanvas();}));
elPanel.appendChild(a);
elPanel.appendChild(s);
elPanel.appendChild(c);
}
function renderWidgetMulti(){
let a=accordion("Multiple Widgets",true);
a.body.appendChild(label(selectedIds.length+" selected"));
a.body.appendChild(input("number","",v=>{
selectedIds.forEach(id=>{
let w=getWidgetById(id);
w.style.font=v;
});
renderCanvas();
}));
...
if(!s)return;
try{let j=JSON.parse(s);data=j;sanitizeClient();markStatus("Imported",true);renderAll();updateActionBar();}
