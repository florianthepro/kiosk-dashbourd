...
#ctxMenu .ctxHead{padding:10px 12px;border-bottom:1px solid #e5e7eb;font-weight:800;background:#f9fafb;color:#111827}
#ctxMenu button[disabled]{opacity:.45;cursor:default}
#ctxMenu button[data-danger]{color:#b91c1c}
#ctxMenu button[data-danger]:hover{background:#fee2e2}
</style>
...
<div id=ctxMenu>
<div class=ctxHead id=ctxHead></div>
<button data-act="addpage">+ Page</button>
<div class=sep data-needs="page"></div>
<button data-act="general" data-needs="page">General…</button>
<button data-act="schedule" data-needs="page">Schedule…</button>
<div class=sep data-needs="page"></div>
<button data-act="rename" data-needs="page">Rename…</button>
<button data-act="duplicate" data-needs="page">Duplicate</button>
<button data-act="delete" data-needs="page" data-danger>Delete</button>
</div>
<div id=wctxMenu>
...
if(elPageList)elPageList.addEventListener("contextmenu",e=>{
if(e.target&&e.target.closest&&e.target.closest(".pg"))return;
ctxShow(e,null);
},true);
...
const elCtxHead=document.getElementById("ctxHead");
...
function ctxHide(){if(!elCtxMenu)return;elCtxMenu.style.display="none";ctxPageIndex=null;ctxUpdate();}
function ctxUpdate(){
if(!elCtxMenu)return;
let hasPage=(ctxPageIndex!=null);
if(elCtxHead){
if(hasPage){let p=data.pages[ctxPageIndex];elCtxHead.textContent=p?(p.name||"Page"):"Page";}
else elCtxHead.textContent="Pages";
}
Array.from(elCtxMenu.querySelectorAll('[data-needs="page"]')).forEach(n=>n.style.display=hasPage?"block":"none");
Array.from(elCtxMenu.querySelectorAll('.sep[data-needs="page"]')).forEach(n=>n.style.display=hasPage?"block":"none");
let bDel=elCtxMenu.querySelector('button[data-act="delete"]');
if(bDel)bDel.disabled=(!hasPage)||data.pages.length<=1;
}
function ctxShow(e,i){
e.preventDefault();e.stopPropagation();
ctxPageIndex=(i==null?null:i);
if(ctxPageIndex!=null){pi=ctxPageIndex;selectedIds=[];renderAll();}
ctxUpdate();
...
let i=ctxPageIndex;
ctxHide();
if(act==="addpage"){addPage();return;}
if(i==null)return;
if(act==="general"){openPageGeneral(i);return;}
if(act==="schedule"){openPageSchedule(i);return;}
if(act==="rename"){let p=data.pages[i];let nn=prompt("Page name:",p.name);if(nn!=null){nn=String(nn).trim();if(nn)p.name=nn;renderAll();}return;}
...
let b=e.target.closest("button");if(!b||b.disabled)return;
let act=b.dataset.act||"";
if(act==="add"){wctxHide();modalShow();return;}
