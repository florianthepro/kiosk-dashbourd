...
<div class=sep></div>
<button data-act="lock" data-needs="sel">Lock/Unlock</button>
<button data-act="hide" data-needs="sel">Show/Hide</button>
<div class=sep></div>
...
function wctxHide(){if(elWCtxMenu)elWCtxMenu.style.display="none";if(elColorPop)elColorPop.style.display="none";}
function wctxUpdate(){
if(!elWCtxMenu)return;
let has=selectedIds&&selectedIds.length>0;
wctxHasSel=has;
Array.from(elWCtxMenu.querySelectorAll('button[data-needs="sel"]')).forEach(b=>b.style.display=has?"flex":"none");
}
...
function ctxHide(){if(!elCtxMenu)return;elCtxMenu.style.display="none";ctxPageIndex=null;}
function ctxShow(e,i){
e.preventDefault();e.stopPropagation();
ctxPageIndex=i;pi=i;selectedIds=[];renderAll();
...
let act=b.dataset.act||"";let i=ctxPageIndex;if(i==null)return;
ctxHide();
...
if(act==="hide"){toggleProp("hidden");return;}
...
</html>""" nun machen wir folgende änderungen: -Lock/Unlock soll der text jewals das anders sein-Show/hide soll nur hide stehen und sowohl im kontext menü wenn man nix angecklickt hat als auch in dem wenn man auf was cklickt soll (sofern ausgeblendete exestiren) im menü unten noch die option hidden und wenn man darauf färt sollman alle hiddenen gelistet haben und wenn man auf eins klickt wird das endhidden-wenn man auf nichts klickt (dh kontext menü nur mit add widget(und hidden) dann sollen die striche im menü nicht genauso viele wie bei dem wenn man was klickt sein
