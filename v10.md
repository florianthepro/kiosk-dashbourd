...
</html>""" der acktuelle code aber erstens noch gequetscht 2. das vorschau past auch nicht (hier als referenz eine ganz alte version wo die vorschau so umgefär gepasst hat: """<?php
header("Content-Type:text/html;charset=utf-8");
$path=__DIR__."/data.json";
$allow_private_fetch=false;
if(($_GET["action"]??"")==="fetchHtml"){
...
</html>""") und die widget buttons wie add sind nur nötig wenn man bei widget ist und wenn man add drückt soll phantom overlay für die funktionen beim adden sein und in Widget soll liste sein mit allen widgets und bei edit selbes overlay und die buttens die im editor oben sind in widgets integriren
