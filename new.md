...
</html>""") ist nun funktionsfähig. bräucht nur noch ein paar änderungen:
-der text rechts bei pages (titel) ist nur teil des ersten buchstabens sichtbar.
-der rand ist schon fix und keine scrollbar allerdings ist der rand breiter dw es rechts unter der menüleiste verschwindet (wäre cooler wenn die als overlay über dem teil rechts mit den pages ist und save/import/export zu pages runter geht (neben "+Page" und "+Rule" verschwindet und man hat da wo davor Save und so hat umschalter zwischen "Pages" und "Editor" )
