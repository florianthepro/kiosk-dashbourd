...
$w=deep_defaults($w,["id"=>$w["id"]??"w".bin2hex(random_bytes(4)),"type"=>$w["type"]??"text","x"=>40,"y"=>40,"w"=>240,"h"=>140,"z"=>1,"locked"=>false,"hidden"=>false,"style"=>["radius"=>0,"shadow"=>false,"bg"=>"","color"=>"","font"=>16],"src"=>"","mode"=>"iframe","text"=>"","format"=>"HH:mm:ss","playlist"=>[],"duration"=>5,"title"=>""]);
...
#pages{width:250px;background:#111827;color:#e5e7eb;overflow:auto;border-right:1px solid #0b1220}
...
#pages .bar button{width:auto;flex:1;background:#1f2937;border:1px solid #273446;color:#e5e7eb;border-radius:6px;padding:6px 8px;cursor:pointer}
#pages .list{padding:6px}
.pg{display:flex;gap:8px;align-items:center;background:#0f172a;border:1px solid #1f2a44;border-radius:8px;padding:8px;margin-bottom:8px;cursor:pointer}
.pg.active{outline:2px solid #60a5fa}
.pg .thumb{width:44px;height:44px;border-radius:6px;background:#0b1220 center/cover no-repeat;border:1px solid #1f2a44;flex:0 0 auto}
...
.pg .acts{display:flex;gap:6px}
.pg .acts button{width:auto;padding:4px 6px;border-radius:6px;background:#111827;border:1px solid #1f2a44;color:#cbd5e1;cursor:pointer}
.pg .acts button:hover{background:#1f2937}
#center{flex:1;display:flex;flex-direction:column;min-width:0}
#centerTop{height:42px;background:#f8fafc;border-bottom:1px solid #d1d5db;display:flex;align-items:center;gap:8px;padding:0 10px}
...
#canvas{position:relative;margin:18px auto;background:#111827;border:1px solid #1f2a44;border-radius:10px}
...
#tabs{display:flex;gap:6px;padding:8px;border-bottom:1px solid #e5e7eb;position:sticky;top:0;background:#fff;z-index:3}
#tabs button{flex:1;background:#f3f4f6;border:1px solid #e5e7eb;border-radius:8px;padding:8px 6px;cursor:pointer;font-weight:600}
...
.btnRow button{flex:1;min-width:110px;background:#111827;color:#fff;border:1px solid #111827;border-radius:8px;padding:8px 10px;cursor:pointer}
...
<span class=pill id=hint>Ctrl/Shift: Multi-Select · Drag: Move · Handle: Resize</span>
<div class=sp></div>
<button onclick=addWidget("text")>+Text</button>
<button onclick=addWidget("image")>+Image</button>
<button onclick=addWidget("url")>+URL</button>
<button onclick=addWidget("clock")>+Clock</button>
<button onclick=addWidget("carousel")>+Carousel</button>
</div>
<div id=canvasWrap><div id=canvas><div id=bgPreview><div class=bgLayer id=bgA></div><div class=bgLayer id=bgB></div></div></div></div>
</div>
<div id=side>
<div id=tabs>
<button id=tGeneral onclick=setTab("general") class=active>General</button>
<button id=tPage onclick=setTab("page")>Page</button>
<button id=tWidget onclick=setTab("widget")>Widget</button>
<button id=tSchedule onclick=setTab("schedule")>Schedule</button>
<button id=tJson onclick=setTab("json")>JSON</button>
</div>
<div id=panel></div>
</div>
</div>
...
function setTab(t){tab=t;["general","page","widget","schedule","json"].forEach(x=>{document.getElementById("t"+x[0].toUpperCase()+x.slice(1)).classList.toggle("active",x===t)});renderSide();}
...
if(!p.settings)p.settings={w:1920,h:1080};
...
if(!w.vp)w.vp={w:3840,h:2160,fit:"contain"};
...
let sub=document.createElement("div");sub.className="sub";sub.textContent=(p.widgets.length||0)+" widgets";
...
acts.appendChild(bUp);acts.appendChild(bDn);acts.appendChild(bDup);acts.appendChild(bDel);
...
function addPage(){
data.pages.push({id:uid(),name:"Page "+(data.pages.length+1),thumb:"",settings:{w:3840,h:2160},background:{fade:1,items:[]},widgets:[]});
...
if(!st||st.sig!==sig){carState[w.id]={sig:sig,i:0,nextAt:0};st=carState[w.id];}
...
let it=w.playlist[i]||{};
el.innerHTML="";
if((it.type||"image")==="image"){let img=document.createElement("img");img.src=it.src||"";el.appendChild(img);}
else{let fr=document.createElement("iframe");fr.src=it.src||"";el.appendChild(fr);}
if(it.title){let t=document.createElement("div");t.className="pvTag";t.textContent=it.title;el.appendChild(t);}
}
...
if(w.type==="url"){
let vp=w.vp||{w:3840,h:2160,fit:"contain"};
let sx=w.w/vp.w,sy=w.h/vp.h;
let s=vp.fit==="cover"?Math.max(sx,sy):Math.min(sx,sy);
let outer=document.createElement("div");outer.style.position="absolute";outer.style.inset="0";outer.style.overflow="hidden";wrap.appendChild(outer);
let inner=document.createElement("div");inner.style.width=vp.w+"px";inner.style.height=vp.h+"px";inner.style.transformOrigin="0 0";inner.style.transform="scale("+s+")";outer.appendChild(inner);
let f=document.createElement("iframe");f.style.width="100%";f.style.height="100%";inner.appendChild(f);
f.src=w.mode==="fetch"?proxyFetchUrl(w.src):w.src;
let tag=document.createElement("div");tag.className="pvTag";tag.textContent=vp.w+"x"+vp.h;wrap.appendChild(tag);
return;
}
if(w.type==="carousel"){
...
let base={id:uid(),type,x:60,y:60,w:320,h:180,z:nextZ(),locked:false,hidden:false,lockAspect:false,vp:{w:3840,h:2160,fit:"contain"},style:{...},src:"",mode:"iframe",text:"",format:"HH:mm:ss",playlist:[]};
...
let cw=p.settings.w,ch=p.settings.h;
w.x=Math.round(clamp(s.x+dx,0,cw-w.w));
w.y=Math.round(clamp(s.y+dy,0,ch-w.h));
});
renderCanvas();
}
if(resizeState){
...
let cw=p.settings.w,ch=p.settings.h;
w.w=Math.round(clamp(resizeState.startW+dw,20,cw-w.x));
w.h=Math.round(clamp(resizeState.startH+dh,20,ch-w.y));
renderCanvas();
}
}
...
let f=fieldset("General Settings");
f.appendChild(label("Timezone (IANA, z.B. Europe/Berlin)"));
f.appendChild(input("text",data.meta.timezone,v=>data.meta.timezone=v));
let r=document.createElement("div");r.className="row2";
let a=document.createElement("div");a.appendChild(label("Auto Reload (hours)"));a.appendChild(input("number",data.meta.autosync_hours,v=>data.meta.autosync_hours=clamp(+v||0,0,168)));
let g=document.createElement("div");g.appendChild(label("Global Rotation Speed (1=normal)"));g.appendChild(input("number",data.meta.globalRotationSpeed,v=>data.meta.globalRotationSpeed=clamp(+v||1,0.1,50)));
r.appendChild(a);r.appendChild(g);
f.appendChild(r);
let f2=fieldset("Hints");
let p=document.createElement("div");p.className="small";p.textContent="Scheduler rules are evaluated in order. Later rules can override background/rotation/widget playlists while a page is active.";
f2.appendChild(p);
elPanel.appendChild(f);elPanel.appendChild(f2);
}
function renderPage(){
let p=getPage();
...
let r=document.createElement("div");r.className="row2";
let cw=document.createElement("div");cw.appendChild(label("Canvas Width"));cw.appendChild(input("number",p.settings.w,v=>{p.settings.w=clamp(+v||1920,320,20000);renderCanvas();}));
let ch=document.createElement("div");ch.appendChild(label("Canvas Height"));ch.appendChild(input("number",p.settings.h,v=>{p.settings.h=clamp(+v||1080,240,20000);renderCanvas();}));
...
let row=document.createElement("div");row.className="row3";
let zIn=input("number",w.z,v=>{w.z=clamp(+v||1,1,99999);renderCanvas();});
let up=btn("▲","alt",()=>{w.z=(w.z||1)+1;renderCanvas();});
let dn=btn("▼","alt",()=>{w.z=Math.max(1,(w.z||1)-1);renderCanvas();});
row.appendChild(zIn);row.appendChild(up);row.appendChild(dn);
z.appendChild(row);
...
if(w.type==="image"){cfg.appendChild(label("Image URL"));cfg.appendChild(input("text",w.src,v=>{w.src=v;renderCanvas();}));}
if(w.type==="url"){
cfg.appendChild(label("Viewport"));
cfg.appendChild(input("number",w.vp.w,v=>{w.vp.w=v;renderCanvas();}));
cfg.appendChild(input("number",w.vp.h,v=>{w.vp.h=v;renderCanvas();}));
cfg.appendChild(select(w.vp.fit,[{v:"contain",t:"contain"},{v:"cover",t:"cover"}],v=>{w.vp.fit=v;renderCanvas();}));
cfg.appendChild(checkbox(w.lockAspect,v=>{w.lockAspect=v;}));
cfg.appendChild(document.createTextNode("Format "+(w.vp.w/w.vp.h).toFixed(2)));
}
...
let f=fieldset("Scheduler Rules");
let s=document.createElement("div");s.className="small";s.textContent="Rules can activate a page and optionally apply overrides (background, rotation speed, widget overrides) during that time window.";
f.appendChild(s);
let br=document.createElement("div");br.className="btnRow";
br.appendChild(btn("+ Rule","",()=>addRule()));
br.appendChild(btn("Sort: Enabled First","alt",()=>{data.schedules.sort((a,b)=>(b.enabled?1:0)-(a.enabled?1:0));renderSide();}));
f.appendChild(br);
data.schedules.forEach((r,i)=>{
let rr=document.createElement("fieldset");rr.style.borderStyle="dashed";
let lg=document.createElement("legend");lg.textContent=(r.enabled?"✓ ":"⨯ ")+r.name;rr.appendChild(lg);
...
let b=document.createElement("div");b.appendChild(label("Enabled"));b.appendChild(checkbox(r.enabled,v=>{r.enabled=v;renderSide();}));
top.appendChild(a);top.appendChild(b);
rr.appendChild(top);
let tm=document.createElement("div");tm.className="row2";
let fr=document.createElement("div");fr.appendChild(label("From (HH:MM)"));fr.appendChild(input("text",r.from,v=>r.from=v));
let to=document.createElement("div");to.appendChild(label("To (HH:MM)"));to.appendChild(input("text",r.to,v=>r.to=v));
tm.appendChild(fr);tm.appendChild(to);
rr.appendChild(tm);
rr.appendChild(label("Weekdays"));
let wd=document.createElement("div");wd.className="row3";
["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].forEach((n,idx)=>{
let d=document.createElement("div");
let c=document.createElement("input");c.type="checkbox";c.checked=r.weekdays.includes(idx+1);
c.oninput=()=>{if(c.checked){if(!r.weekdays.includes(idx+1))r.weekdays.push(idx+1);}else r.weekdays=r.weekdays.filter(x=>x!==idx+1);};
...
rr.appendChild(wd);
rr.appendChild(label("Activate Page (optional)"));
let opts=data.pages.map(p=>({v:p.id,t:p.name}));opts.unshift({v:"",t:"(no page change)"});
rr.appendChild(select(r.page||"",opts,v=>r.page=v));
let ov=fieldset("Overrides (optional)");
let sp=document.createElement("div");sp.className="row2";
let rs=document.createElement("div");rs.appendChild(label("Rotation Speed Multiplier"));rs.appendChild(input("number",r.override.rotationSpeed??"",v=>{r.override.rotationSpeed=(v===""||isNaN(v))?null:clamp(+v||1,0.1,50);}));
let clr=document.createElement("div");clr.appendChild(label("Reset Override"));clr.appendChild(btn("Clear Overrides","alt",()=>{r.override={rotationSpeed:null,background:null,widgets:{}};renderSide();}));
sp.appendChild(rs);sp.appendChild(clr);
ov.appendChild(sp);
let obg=fieldset("Override Background Playlist");
let obgOn=document.createElement("div");obgOn.className="row2";
let bgToggle=document.createElement("div");bgToggle.appendChild(label("Enable Background Override"));
bgToggle.appendChild(checkbox(!!r.override.background,v=>{if(v){r.override.background={fade:1,items:[{src:"https://",duration:10}]};}else r.override.background=null;renderSide();}));
let bgFade=document.createElement("div");bgFade.appendChild(label("Fade (s)"));bgFade.appendChild(input("number",r.override.background?r.override.background.fade:1,v=>{if(r.override.background)r.override.background.fade=clamp(+v||0,0,20);}));
obgOn.appendChild(bgToggle);obgOn.appendChild(bgFade);
obg.appendChild(obgOn);
if(r.override.background){
obg.appendChild(btn("+ Item","",()=>{r.override.background.items.push({src:"https://",duration:10});renderSide();}));
r.override.background.items.forEach((it,bi)=>{
let biF=document.createElement("fieldset");biF.style.borderStyle="dashed";let lg2=document.createElement("legend");lg2.textContent="BG Item "+(bi+1);biF.appendChild(lg2);
biF.appendChild(label("Image URL"));biF.appendChild(input("text",it.src,v=>it.src=v));
biF.appendChild(label("Duration (s)"));biF.appendChild(input("number",it.duration,v=>it.duration=clamp(+v||1,1,3600)));
let bbr=document.createElement("div");bbr.className="btnRow";
bbr.appendChild(btn("Up","alt",()=>{if(bi===0)return;let a=r.override.background.items[bi-1];r.override.background.items[bi-1]=r.override.background.items[bi];r.override.background.items[bi]=a;renderSide();}));
bbr.appendChild(btn("Down","alt",()=>{if(bi===r.override.background.items.length-1)return;let a=r.override.background.items[bi+1];r.override.background.items[bi+1]=r.override.background.items[bi];r.override.background.items[bi]=a;renderSide();}));
bbr.appendChild(btn("Delete","danger",()=>{r.override.background.items.splice(bi,1);renderSide();}));
biF.appendChild(bbr);
obg.appendChild(biF);
});
}
ov.appendChild(obg);
let ow=fieldset("Override Widgets (for selected page)");
let wopts=(data.pages.find(p=>p.id===r.page)||getPage()).widgets.map(w=>({v:w.id,t:w.type+" · "+(w.text||w.src||w.id).slice(0,22)}));
if(!wopts.length){let t=document.createElement("div");t.className="small";t.textContent="No widgets available on that page.";ow.appendChild(t);}
else{
let pick=select("",[{v:"",t:"(choose widget)"}].concat(wopts),v=>{
if(!v)return;
if(!r.override.widgets)r.override.widgets={};
if(!r.override.widgets[v])r.override.widgets[v]={hidden:null,src:null,text:null,playlist:null,style:null,z:null,speed:null};
renderSide();
});
ow.appendChild(label("Add Widget Override"));
ow.appendChild(pick);
Object.keys(r.override.widgets||{}).forEach(wid=>{
let conf=r.override.widgets[wid];
let basePage=data.pages.find(p=>p.id===r.page)||getPage();
let baseW=basePage.widgets.find(x=>x.id===wid);
let wf=document.createElement("fieldset");wf.style.borderStyle="dashed";
let lg=document.createElement("legend");lg.textContent="Widget "+(baseW?baseW.type:"")+" · "+wid;wf.appendChild(lg);
let tr=document.createElement("div");tr.className="row2";
let hid=document.createElement("div");hid.appendChild(label("Hidden (override)"));
hid.appendChild(select(conf.hidden===null?"":(conf.hidden?"1":"0"),[{v:"",t:"(no override)"},{v:"1",t:"hide"},{v:"0",t:"show"}],v=>{conf.hidden=v===""?null:(v==="1");}));
let zz=document.createElement("div");zz.appendChild(label("Z (override)"));
zz.appendChild(input("number",conf.z??"",v=>{conf.z=(v===""||isNaN(v))?null:clamp(+v||1,1,99999);}));
tr.appendChild(hid);tr.appendChild(zz);
wf.appendChild(tr);
wf.appendChild(label("Speed Multiplier (override, affects carousel timing in future index.php)"));
wf.appendChild(input("number",conf.speed??"",v=>{conf.speed=(v===""||isNaN(v))?null:clamp(+v||1,0.1,50);}));
if(baseW&&(baseW.type==="image"||baseW.type==="url")){wf.appendChild(label("Src override"));wf.appendChild(input("text",conf.src??"",v=>{conf.src=v===""?null:v;}));}
if(baseW&&baseW.type==="text"){wf.appendChild(label("Text override"));wf.appendChild(textarea(conf.text??"",v=>{conf.text=v===""?null:v;}));}
if(baseW&&baseW.type==="carousel"){
wf.appendChild(label("Playlist override (replaces)"));
wf.appendChild(checkbox(!!conf.playlist,v=>{conf.playlist=v?([{type:"image",src:"https://",duration:5,title:""}]):null;renderSide();}));
if(conf.playlist){
wf.appendChild(btn("+ Item","",()=>{conf.playlist.push({type:"image",src:"https://",duration:5,title:""});renderSide();}));
conf.playlist.forEach((it,ii)=>{
let fi=document.createElement("fieldset");fi.style.borderStyle="dashed";let lg3=document.createElement("legend");lg3.textContent="Item "+(ii+1);fi.appendChild(lg3);
fi.appendChild(label("Type"));fi.appendChild(select(it.type||"image",[{v:"image",t:"image"},{v:"url",t:"url"}],v=>it.type=v));
fi.appendChild(label("Src"));fi.appendChild(input("text",it.src||"",v=>it.src=v));
fi.appendChild(label("Title"));fi.appendChild(input("text",it.title||"",v=>it.title=v));
fi.appendChild(label("Duration (s)"));fi.appendChild(input("number",it.duration||5,v=>it.duration=clamp(+v||1,1,3600)));
let bbr=document.createElement("div");bbr.className="btnRow";
bbr.appendChild(btn("Up","alt",()=>{if(ii===0)return;let a=conf.playlist[ii-1];conf.playlist[ii-1]=conf.playlist[ii];conf.playlist[ii]=a;renderSide();}));
bbr.appendChild(btn("Down","alt",()=>{if(ii===conf.playlist.length-1)return;let a=conf.playlist[ii+1];conf.playlist[ii+1]=conf.playlist[ii];conf.playlist[ii]=a;renderSide();}));
bbr.appendChild(btn("Delete","danger",()=>{conf.playlist.splice(ii,1);renderSide();}));
fi.appendChild(bbr);
wf.appendChild(fi);
});
}
}
wf.appendChild(label("Style override (optional, JSON object)"));
wf.appendChild(textarea(conf.style?JSON.stringify(conf.style):"",v=>{try{conf.style=v.trim()?JSON.parse(v):null;}catch(e){}}));
wf.appendChild(btn("Remove Widget Override","danger",()=>{delete r.override.widgets[wid];renderSide();}));
ow.appendChild(wf);
});
}
ov.appendChild(ow);
rr.appendChild(ov);
let rrBtns=document.createElement("div");rrBtns.className="btnRow";
rrBtns.appendChild(btn("Up","alt",()=>moveRule(i,-1)));
rrBtns.appendChild(btn("Down","alt",()=>moveRule(i,1)));
rrBtns.appendChild(btn("Duplicate","alt",()=>{let c=deepClone(r);c.id=uid();c.name=c.name+" Copy";data.schedules.splice(i+1,0,c);renderSide();}));
rrBtns.appendChild(btn("Delete","danger",()=>{data.schedules.splice(i,1);renderSide();}));
rr.appendChild(rrBtns);
f.appendChild(rr);
});
elPanel.appendChild(f);
}
function renderJson(){
let f=fieldset("Raw JSON");
let t=document.createElement("div");t.className="small";t.textContent="You can edit JSON directly. Invalid JSON will not be saved.";
f.appendChild(t);
let ta=document.createElement("textarea");ta.style.minHeight="380px";ta.value=JSON.stringify(data,null,2);
ta.oninput=()=>{};
let br=document.createElement("div");br.className="btnRow";
br.appendChild(btn("Apply JSON","",()=>{
try{let j=JSON.parse(ta.value);data=j;sanitizeClient();markStatus("JSON applied",true);renderAll();}
catch(e){markStatus("Invalid JSON",false);}
}));
br.appendChild(btn("Refresh View","alt",()=>{ta.value=JSON.stringify(data,null,2);markStatus("Refreshed",true);}));
f.appendChild(ta);f.appendChild(br);
elPanel.appendChild(f);
}
...
if(!s)return;
try{let j=JSON.parse(s);data=j;sanitizeClient();markStatus("Imported",true);renderAll();setTab("json");}
