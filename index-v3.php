...
<div id=wrap>
<div id=scale>
<div id=baseStage class=stage>
<div class=bgWrap><div id=baseBgA class=bg></div><div id=baseBgB class=bg></div></div><div id=baseLayer class=layer></div></div><div id=overlayStage class=stage><div class=bgWrap><div id=ovBgA class=bg></div><div id=ovBgB class=bg></div></div><div id=ovLayer class=layer></div></div></div></div><script>
...
function applyView(){let cw=stageW,ch=stageH;let s=Math.min(Math.max(100,innerWidth)/cw,Math.max(100,innerHeight)/ch);s=clamp(s,0.05,10);viewScale=s;elScale.style.width=Math.round(cw*s)+"px";elScale.style.height=Math.round(ch*s)+"px";for(let st of [elBaseStage,elOverlayStage]){st.style.width=cw+"px";st.style.height=ch+"px";st.style.transform="scale("+s+")";}}
...
function tickCars(cars){let now=Date.now();for(let id in cars){let st=cars[id];if(!st||!st.list||!st.list.length)continue;if(st.nextAt===0){st.nextAt=now+carouselDurationMs(st);continue;}if(now<st.nextAt)continue;carouselShow(st,(st.i+1)%st.list.length);st.nextAt=now+carouselDurationMs(st);}}
