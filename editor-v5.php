...
<style><style:#e9edf2;--ink:#0f172a;--muted:#64748b;--line:#d1d5db;--panel:#ffffff;--panel2:#f8fafc;--dark:#111827;--dark2:#0b1220;--dark3:#0f172a;--btn:#e5e7eb;--btnLine:#9ca3af;--accent:#2563eb;--danger:#dc2626}
html,body{margin:0;height:100%;font-family:system-ui,Segoe UI,Arial;background:var(--bg);color:var(--ink)}
#top{height:46px;background:#fff;color:var(--ink);display:flex;align-items:center;gap:8px;padding:0 12px;border-bottom:1px solid var(--line)}
#top .sp{flex:1}
#top button{background:var(--btn);border:1px solid var(--btnLine);color:var(--ink);border-radius:6px;padding:7px 10px;cursor:pointer}
#top button:hover{background:#fff}
#status.badge{background:var(--panel2);border:1px solid var(--line);color:var(--ink)}
#main{display:grid;grid-template-columns:280px 1fr;height:calc(100% - 46px)}
#pages{grid-column:1;background:var(--panel);color:var(--ink);overflow:hidden;border-right:1px solid var(--line);display:flex;flex-direction:column}
#center{grid-column:2;display:flex;flex-direction:column;min-width:0;background:var(--panel2);min-height:0;overflow:hidden}
#pages .bar{display:flex;gap:6px;flex-wrap:wrap;padding:8px;border-bottom:1px solid var(--line);position:sticky;top:0;background:var(--panel);z-index:2;flex:0 0 auto}
#pages .bar button{width:auto;flex:0 0 auto;background:var(--btn);border:1px solid var(--btnLine);color:var(--ink);border-radius:6px;padding:6px 8px;cursor:pointer}
#pages .bar button:hover{background:#fff}
#pageList{flex:1;overflow:auto}
#pages .list{padding:8px}
.pg{display:grid;grid-template-columns:44px 1fr;grid-template-rows:auto auto;gap:8px;align-items:start;background:#fff;border:1px solid var(--line);border-radius:10px;padding:8px;margin-bottom:8px;cursor:pointer}
.pg.active{outline:2px solid rgba(37,99,235,.55)}
.pg .thumb{width:44px;height:44px;border-radius:8px;background:#fff center/cover no-repeat;border:1px solid var(--line);grid-row:1 / span 2;grid-column:1}
.pg .meta{grid-row:1;grid-column:2;min-width:0}
.pg .meta .name{font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.pg .meta .sub{font-size:12px;color:var(--muted)}
.pg .acts{grid-row:2;grid-column:2;display:flex;gap:6px;justify-self:end}
.pg .acts button{width:24px;height:24px;padding:0;border-radius:6px;background:var(--btn);border:1px solid var(--btnLine);color:var(--ink);cursor:pointer;line-height:22px}
.pg .acts button:hover{background:#fff}
...
#centerTop{height:34px;background:var(--panel);border-bottom:1px solid var(--line);display:flex;align-items:center;gap:8px;padding:0 10px;flex:0 0 auto}
#centerTop .pill{font-size:12px;color:var(--ink);background:var(--panel2);border:1px solid var(--line);border-radius:999px;padding:5px 10px}
#canvasWrap{flex:1;min-height:0;min-width:0;position:relative;background:#000;overflow:hidden;display:flex;align-items:center;justify-content:center;box-sizing:border-box;padding:0 10px 10px 0}
#canvasScale{position:relative}
#canvas{transform-origin:0 0;position:relative;margin:0;background:#111827;border:2px solid #0b1220;border-radius:8px;box-shadow:0 10px 30px rgba(0,0,0,.25)}
...
.widget{position:absolute;box-sizing:border-box;border:1px solid rgba(37,99,235,.65);background:rgba(255,255,255,.92);color:var(--ink);overflow:hidden;z-index:1;border-radius:8px}
.widget.sel{outline:2px solid rgba(220,38,38,.85)}
.widget.locked{border-style:dashed;opacity:.9}
...
.pvTag{position:absolute;left:6px;top:6px;font-size:11px;background:rgba(0,0,0,.55);color:#fff;padding:2px 8px;border-radius:999px;pointer-events:none}
.pvLayer{position:absolute;right:6px;top:6px;font-size:11px;background:rgba(0,0,0,.55);color:#fff;padding:2px 8px;border-radius:999px;pointer-events:none}
#tabs{flex:0 0 auto;display:flex;gap:6px;padding:8px;border-bottom:1px solid var(--line);background:var(--panel);position:static}
#tabs button{flex:1;background:var(--btn);border:1px solid var(--btnLine);border-radius:8px;padding:8px 6px;color:var(--ink);cursor:pointer;font-weight:700}
#tabs button.active{background:#fff;border-color:var(--accent);box-shadow:0 0 0 2px rgba(37,99,235,.15) inset}
#panel{flex:1;overflow:auto;padding:10px;background:#fff}
fieldset{border:1px solid var(--line);border-radius:12px;padding:10px;margin:0 0 10px;background:#fff}
legend{padding:0 8px;color:var(--ink);font-weight:800}
label{display:block;font-size:12px;color:var(--muted);margin:8px 0 4px}
input,select,textarea{width:100%;box-sizing:border-box;border:1px solid var(--line);border-radius:10px;padding:8px;font:inherit;background:#fff}
textarea{min-height:70px;resize:vertical}
.row2{display:grid;grid-template-columns:1fr 1fr;gap:8px}
.row3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px}
.btnRow{display:flex;gap:8px;flex-wrap:wrap}
.btnRow button{flex:1;min-width:88px;background:var(--ink);color:#fff;border:1px solid var(--ink);border-radius:10px;padding:8px 10px;cursor:pointer}
.btnRow button.alt{background:#fff;color:var(--ink);border-color:var(--line)}
.btnRow button.danger{background:var(--danger);border-color:var(--danger)}
.small{font-size:12px;color:var(--muted)}
.badge{display:inline-block;font-size:11px;border:1px solid var(--line);border-radius:999px;padding:3px 8px;background:var(--panel2);color:var(--ink)}
#pagesModeHint{display:none!important}
#editorPanel .bar{display:flex;gap:6px;flex-wrap:wrap;flex:0 0 auto;padding:8px;border-bottom:1px solid var(--line);background:var(--panel)}
#editorPanel .bar button{flex:1 1 90px;background:var(--btn);border:1px solid var(--btnLine);color:var(--ink);border-radius:8px;padding:6px 8px;cursor:pointer;font-weight:700}
#editorPanel .bar button:hover{background:#fff}
#widgetListLeft{flex:0 0 220px;overflow:auto;padding:8px;border-top:1px solid var(--line);background:var(--panel)}
.wli{display:flex;align-items:center;gap:8px;background:#fff;border:1px solid var(--line);border-radius:10px;padding:8px;margin-bottom:8px;cursor:pointer}
.wli.sel{outline:2px solid rgba(220,38,38,.6)}
.wli .t{flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--ink);font-weight:700}
.wli .z{font-size:11px;color:var(--muted)}
...
#modal{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);width:min(520px,92vw);max-height:min(75vh,700px);background:#fff;border:1px solid var(--line);border-radius:12px;box-shadow:0 14px 40px rgba(0,0,0,.35);z-index:9999;display:none;flex-direction:column}
#modalHead{padding:10px 12px;border-bottom:1px solid var(--line);font-weight:900}
#modalBody{padding:10px 12px;overflow:auto}
#modalFoot{display:flex;gap:8px;padding:10px 12px;border-top:1px solid var(--line)}
#modalFoot button{flex:1;background:var(--ink);color:#fff;border:1px solid var(--ink);border-radius:10px;padding:8px 10px;cursor:pointer}
#modalCancel{background:#fff!important;color:var(--ink)!important;border-color:var(--line)!important}
</style>
