...
$icon=safe_href(($data["meta"]["icon"]??($data["meta"]["favicon"]??""))?:($data["pages"][0]["thumb"]??""));
...
<?php $t=(string)($data["pages"][0]["name"]??""); if($t!==""){ ?><title><?php echo htmlspecialchars($t,ENT_QUOTES|ENT_SUBSTITUTE,"UTF-8"); ?></title><?php } ?>
<?php if($icon!==""){ ?><link rel="icon" href="<?php echo htmlspecialchars($icon,ENT_QUOTES|ENT_SUBSTITUTE,"UTF-8"); ?>"><?php } ?>
<style>
...
.widget.carousel iframe{background:#000}
</style>
</head>
<body>
<div id=wrap><div id=stageScale><div id=stage><div id=bg><div class="bgLayer" id=bgA></div><div class="bgLayer" id=bgB></div></div><div id=w></div></div></div></div>
<script>
...
if(!list.length){box.textContent="empty";}
else{
let stage=document.createElement("div");
stage.style.position="absolute";
stage.style.inset="0";
box.appendChild(stage);
carState[w.id]={i:0,nextAt:0,list:list,box:box,stage:stage,nodes:[],activeNode:null};
carouselShow(carState[w.id],0);
carState[w.id].nextAt=Date.now()+carouselDurationMs(carState[w.id]);
}
}
function tickCarousels(){
let now=Date.now();
for(let wid in carState){
let st=carState[wid];
