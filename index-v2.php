...
function json_path(){return file_exists(__DIR__."/data/data.json")?__DIR__."/data/data.json":__DIR__."/data.json";}
...
if(!isset($r["override"]["widgets"]))$r["override"]["widgets"]=[];
}
return $d;
}
...
$icon=safe_href($meta["icon"]??($meta["favicon"]??""));
if($icon==="")$icon=safe_href($data["pages"][0]["thumb"]??"");
$theme=(string)($meta["themeColor"]??"");
?><!doctype html><html><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><?php if($theme!==""){?><meta name=theme-color content="<?php echo htmlspecialchars($theme,ENT_QUOTES|ENT_SUBSTITUTE,"UTF-8"); ?>"><?php } ?><?php if($title!==""){?><title><?php echo htmlspecialchars($title,ENT_QUOTES|ENT_SUBSTITUTE,"UTF-8"); ?></title><?php } ?><?php if($icon!==""){?><link rel="icon" href="<?php echo htmlspecialchars($icon,ENT_QUOTES|ENT_SUBSTITUTE,"UTF-8"); ?>"><?php } ?><style>html,body{margin:0;height:100%;background:#000;overflow:hidden;font-family:system-ui,Segoe UI,Arial}#wrap{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:#000}#scale{position:relative}.stage{position:absolute;left:0;top:0;transform-origin:0 0}.bgWrap{position:absolute;inset:0;overflow:hidden}.bg{position:absolute;inset:0;background:#000 center/cover no-repeat;opacity:0;transition:opacity 1s}.bg.on{opacity:1}.layer{position:absolute;inset:0}.w{position:absolute;box-sizing:border-box;overflow:hidden}.w.hidden{display:none}.w .inner{position:absolute;inset:0}.w.text .inner{white-space:pre-wrap;padding:10px}.w.clock .inner{display:flex;align-items:center;justify-content:center}.w.image img{width:100%;height:100%;object-fit:cover;display:block}.w.url .frameWrap{position:absolute;left:0;top:0;transform-origin:0 0}.w.url iframe{border:0;display:block;background:#fff}.w.carousel .slot{position:absolute;inset:0}.w.carousel img,.w.carousel iframe{width:100%;height:100%;border:0;display:block}#overlayStage{opacity:0;pointer-events:none;transition:opacity .25s}#overlayStage.on{opacity:1}</style></head><body><div id=wrap><div id=scale><div id=baseStage class=stage><div class=bgWrap><div id=baseBgA class=bg></div><div id=baseBgB class=bg></div></div><div id=baseLayer class=layer></div></div><div id=overlayStage class=stage><div class=bgWrap><div id=ovBgA class=bg></div><div id=ovBgB class=bg></div></div><div id=ovLayer class=layer></div></div></div></div><script>
...
let overlayIdx=0;
let nextOverlayAt=0;
let baseBgState={sig:"",i:0,nextAt:0,onA:true};
let ovBgState={sig:"",i:0,nextAt:0,onA:true};
let baseClockNodes=[];
let ovClockNodes=[];
let baseCars={};
let ovCars={};
...
function bgSetup(p,a,b,state){let fade=clamp(+((p.background&&p.background.fade)||0),0,20);a.style.transitionDuration=fade+"s";b.style.transitionDuration=fade+"s";let sig=bgSig(p);if(state.sig!==sig){state.sig=sig;state.i=0;state.nextAt=0;state.onA=true;}}
...
function renderUrl(inner,w){let url=(w.src||"").trim();if(!url)return;let mode=(w.mode||"iframe")==="fetch"?"fetch":"iframe";let cw=+w.contentW||stageW;let ch=+w.contentH||stageH;let wrap=document.createElement("div");wrap.className="frameWrap";wrap.style.width=cw+"px";wrap.style.height=ch+"px";let sc=Math.min((w.w||1)/cw,(w.h||1)/ch);sc=clamp(sc,0.01,10);wrap.style.transform="scale("+sc+")";let fr=document.createElement("iframe");fr.width=cw;fr.height=ch;fr.style.width=cw+"px";fr.style.height=ch+"px";fr.style.border="0";fr.loading="eager";fr.allow="fullscreen";if(mode==="fetch")fr.setAttribute("sandbox","allow-scripts allow-forms allow-popups allow-same-origin allow-popups-to-escape-sandbox allow-top-navigation-by-user-activation");fr.src=(mode==="fetch")?proxyFetchUrl(url):url;wrap.appendChild(fr);inner.appendChild(wrap);}
function carouselEnsureNode(st,idx){if(st.nodes[idx])return st.nodes[idx];let it=st.list[idx]||{};let t=(it.type||"image");let node;if(t==="image"){node=new Image();node.src=it.src||"";}else{node=document.createElement("iframe");node.src=it.src||"";node.loading="eager";node.allow="fullscreen";node.style.border="0";}node.className="slot";st.nodes[idx]=node;return node;}
function carouselShow(st,idx){idx=((idx%st.list.length)+st.list.length)%st.list.length;st.box.innerHTML="";st.box.appendChild(carouselEnsureNode(st,idx));st.i=idx;}
function tickCars(cars){let now=Date.now();for(let id in cars){let st=cars[id];if(!st||!st.list||!st.list.length)continue;if(st.nextAt===0){let dur=+((st.list[st.i]||{}).duration||5);dur=clamp(dur,1,3600);st.nextAt=now+dur*1000;}if(now<st.nextAt)continue;carouselShow(st,(st.i+1)%st.list.length);let dur=+((st.list[st.i]||{}).duration||5);dur=clamp(dur,1,3600);st.nextAt=now+dur*1000;}}
...
function renderOverlay(){if(!overlayPages.length){showOverlay(false);ovPageId="";ovClockNodes=[];ovCars={};return;}showOverlay(true);let p=effectivePage(overlayPages[overlayIdx]);stageW=+((p.settings&&p.settings.w)||3840)||3840;stageH=+((p.settings&&p.settings.h)||2160)||2160;applyView();bgSetup(p,ovBgA,ovBgB,ovBgState);bgTick(p,ovBgA,ovBgB,ovBgState);let r=renderStage(elOvLayer,p);ovClockNodes=r.clocks;ovCars=r.cars;ovPageId=p.id;}
function tick(){if(basePage){bgTick(basePage,baseBgA,baseBgB,baseBgState);tickClocks(baseClockNodes);tickCars(baseCars);}computeOverlayPages();if(!overlayPages.length){showOverlay(false);return;}if(ovPageId===""||ovPageId!==overlayPages[overlayIdx].id){nextOverlayAt=Date.now()+rotationMs();renderOverlay();}let p=effectivePage(overlayPages[overlayIdx]);bgTick(p,ovBgA,ovBgB,ovBgState);tickClocks(ovClockNodes);tickCars(ovCars);let now=Date.now();if(nextOverlayAt===0)nextOverlayAt=now+rotationMs();if(now>=nextOverlayAt){overlayIdx=(overlayIdx+1)%overlayPages.length;renderOverlay();nextOverlayAt=now+rotationMs();}}
async function reloadIfChanged(){try{let r=await fetch("<?php echo htmlspecialchars(basename($path),ENT_QUOTES|ENT_SUBSTITUTE,"UTF-8"); ?>?ts="+Date.now(),{cache:"no-store"});if(!r.ok)return;let j=await r.json();if(JSON.stringify(j)===JSON.stringify(DATA))return;DATA=j;buildBasePage();computeOverlayPages();renderBase();renderOverlay();}catch(e){}}
function start(){buildBasePage();renderBase();computeOverlayPages();renderOverlay();setInterval(tick,250);let ah=+((DATA.meta&&DATA.meta.autosync_hours)||6)||6;let iv=clamp(ah*3600*1000,30000,24*3600*1000);setInterval(reloadIfChanged,iv);}
start();
</script></body></html>
